...
public class HttpClient {
  ...
  public static final String USER_AGENT = format("appnexus-java/%s", Appnexus.VERSION);
  ...
  protected String request(RequestMethod method, String url, Map<String, String> headers, String payload) {
    ...
    try {
      ...
      if (method.equals(RequestMethod.GET)) {
        ...
        throw new AppnexusException(format("Unsupported Request Method [method: %s, url: %s]", method, url));
      }
      int statusCode = conn.getResponseCode(); //triggers the request
      if (statusCode < 200 || statusCode >= 300) {
        throw new AppnexusException(url, statusCode, getResponse(conn.getErrorStream()));
      }    
      return getResponse(conn.getInputStream());
    } catch (Exception ex) {
      throw new AppnexusException(format("Failed request [url: %s]", url), ex);
    } finally {
      if (conn != null) {
        conn.disconnect();
      }
